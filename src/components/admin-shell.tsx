...
import { BookOpen, Image, LogOut, Sparkles, UsersRound } from 'lucide-react';
...
export function AdminShell({ children, onSignedOut }: { children: React.ReactNode; onSignedOut?: () => void }) {
  ...
  useEffect(() => {
    if (!authConfigured() || !configureAuth()) { setSessionReady(true); return; }
    let active = true;
    async function verify() { try { const session = await fetchAuthSession(); if (!session.tokens?.idToken) throw new Error('Session expired'); if (active) setSessionReady(true); } catch { if (active) window.location.assign('/'); } }
