...
export async function GET(request: Request) {
  ...
  const videos = await Promise.all(page.filter(doc => doc.data().status === 'Published').map(async doc => {
    const data = doc.data();
    return { id: doc.id, title: data.title, description: data.description ?? '', uploader: data.uploaderEmail, thumbnailUrl: await signedVideoUrl(data.thumbnailKey), upvotes: data.upvotes ?? 0, downvotes: data.downvotes ?? 0, commentCount: data.commentCount ?? 0, createdAt: data.createdAt?.toDate?.().toISOString() ?? null };
  }));
...
export async function POST(request: Request) {
  try {
    ...
    const data = { title: title.trim(), description: description.trim(), videoKey, thumbnailKey, uploaderEmail: player.email, uploaderId: player.id, status: 'Published', upvotes: 0, downvotes: 0, commentCount: 0, createdAt: FieldValue.serverTimestamp() };
