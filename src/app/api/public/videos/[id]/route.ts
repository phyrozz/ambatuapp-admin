...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  return NextResponse.json({ id, title: data.title, description: data.description ?? '', uploader: data.uploaderEmail, videoUrl: await signedVideoUrl(data.videoKey), upvotes: data.upvotes ?? 0, downvotes: data.downvotes ?? 0, commentCount: data.commentCount ?? 0 }, { headers });
}
