...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  return NextResponse.json({ comments: snapshot.docs.map(doc => { const data = doc.data(); return { id: doc.id, text: data.text, author: data.author ?? 'Anonymous', createdAt: data.createdAt?.toDate?.().toISOString() ?? null }; }) }, { headers });
}
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const author = player?.email ?? 'Anonymous';
    const ref = await video.collection('comments').add({ text: text.trim(), author, authorId: player?.id ?? anonymousId, createdAt: FieldValue.serverTimestamp() });
